...
class WidgetRepositoryEloquent implements WidgetRepository {
	...
	public function all($columns = ['*']) {
		try {
			$res = Cache::tags(['widgets'])->remember('widgets.all', Config::get('cache.duration.hour'), function() use($columns) {
	...
	public function find($id, $columns = ['*']) {
		try {
			return Cache::tags(['widgets'])->remember('widgets.find.' . $id, Config::get('cache.duration.hour'), function() use($id, $columns) {
	...
	public function findBy($key, $val, $columns = ['*']) {
		try {
			return Cache::tags(['widgets'])->remember('widgets.findBy.' . $key . '.' . str_slug($val), Config::get('cache.duration.hour'), function() use($key, $val, $columns) {
	...
	public function selectList($key, $val) {
		try {
			$res = Cache::tags(['widgets'])->remember('widgets.selectList.' . $key . '.' . $val, Config::get('cache.duration.hour'), function() use($val) {
